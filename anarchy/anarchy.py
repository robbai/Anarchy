...
from utilities.calculations import invert_angle, get_car_facing_vector, get_ball_bounces, \
    get_impact, distance_from_wall, inside_turning_radius, project_to_wall, closest_boost
...
class Anarchy(BaseAgent):
    ...
    def get_output(self, packet: GameTickPacket) -> SimpleControllerState:
        ...
        team_sign = (1 if self.car.team == (is_1v1 and packet.teams[self.car.team].score == 7 and
                                            packet.teams[not self.car.team].score == 0) else -1)
        enemy_goal = Vector2(0, team_sign * 5120)
        kickoff = packet.game_info.is_kickoff_pause
        ball_prediction = self.get_ball_prediction_struct()
        impacts = [get_impact(ball_prediction, car, self.time, self.renderer)
        ...
        rotation_matrix = Matrix3D([self.car.physics.rotation.pitch, self.car.physics.rotation.yaw,
        ...
        if self.aerial is not None:
            if self.car.has_wheel_contact and self.aerial.jt + 0.5 < self.time:
                ...
                aerial_output = self.aerial.execute(packet, self.index,
                                                    self.get_ball_prediction_struct())
                if self.aerial.target is not None:
                    self.renderer.begin_rendering()
                    self.renderer.draw_line_3d(car_location, car_location + self.aerial.target,
                                               self.renderer.white())
                    self.renderer.end_rendering()
        ...
        for index, car in enumerate(packet.game_cars[:packet.num_cars]):
            if car.team == self.team and index != self.index:
                ...
                if correct_side_of_ball and not teammate_correct_side_of_ball:
                    continue
                if impacts[index][1] + 0.1 < impact_time or (not correct_side_of_ball and
                                                             teammate_correct_side_of_ball):
        ...
        close_boost = closest_boost(Vector3(-impact.x, impact.y, 0) if teammate_going_for_ball and
                                    impact_time < 2.5 and not kickoff else car_location,
                                    self.get_field_info().boost_pads, packet.game_boosts)
        convenient_boost = (close_boost and (close_boost - car_location).flatten().length *
                            (5 if is_1v1 else 3) < (impact - car_location).flatten().length)
        ...
        if kickoff and not not_our_kickoff:
        ...
        elif avoid_own_goal and not kickoff and not demoing and (not teammate_going_for_ball or
                                                                 impact.y * team_sign > -4000):
            offset = (max(0, impact_time - 0.5) * 160 + 110)
            destination += Vector2(offset * -sign(impact_projection.x),
                                   (140 if wait else 0) * -team_sign)
        elif not_our_kickoff or teammate_going_for_ball or demoing or \
                (close_boost and need_boost and (not is_1v1 or convenient_boost)):
            if close_boost:
            ...
            if demo_location is None:
                self.demo = None
                demoing = False
                if (not need_boost and car_to_ball.length > 2500) if not not_our_kickoff else (
            ...
            destination.x += max(car_to_ball.length / 3, 80 if wait else 120) * - \
                sign(impact_projection.x)
        else:
            destination += (destination - enemy_goal).normalized * max(
                (destination - car_location).length /
                ((2.1 + impact_time / 1.9) if take_serious_shot else 3.7), 60 if wait else 110)
        ...
        self.renderer.draw_line_3d([destination.x, destination.y, impact.z], [
                                   impact.x, impact.y, impact.z], self.renderer.blue())
        if avoid_own_goal:
            self.renderer.draw_line_3d([car_location.x, car_location.y, 0], [
        ...
        local = rotation_matrix.dot(Vector3(car_to_destination.x, car_to_destination.y,
        ...
        slow_down = (abs(self.steer_correction_radians) > 0.2 and inside_turning_radius(
            local, car_velocity.length) and obey_turning_radius)
        if slow_down:
            self.renderer.begin_rendering()
            self.renderer.draw_string_2d(triforce(20, 50), triforce(
                10, 20), 6, 6, 'Uh Oh',
        ...
        if park_car:
            ...
            if car_velocity.length > 400:
                self.controller.throttle = - \
                    sign(rotation_matrix.data[0].dot(destination - car_location))
            ...
            self.controller.boost = (abs(turning_radians) <
        ...
        self.controller.handbrake = (abs(turning_radians) > 1.1 and
                                     not self.car.is_super_sonic and not (slow_down or park_car))
        ...
        if sign(rotation_velocity.z) != sign(self.controller.steer) or \
                sign(car_local_velocity.x) != sign(self.controller.throttle):
            self.controller.handbrake = False

        # Dodging
        self.controller.jump = False
        dodge_for_speed = (velocity_change > 500 and not backwards and self.car.boost < 14 and
                           self.time > self.next_dodge_time + 0.85 and
                           car_to_destination.size > (2000 if take_serious_shot else 1200) and
                           abs(turning_radians) < 0.1 and not demoing)
        if kickoff:
            dodge_for_speed = car_velocity.size > 1250
        if (((car_to_ball.flatten().size < 400 and ball_location.z < 300) or dodge_for_speed) and
                car_velocity.size > 1100) or self.dodging:
        ...
        # Half-flips
        if backwards and not park_car and (time if wait else impact_time) > 0.6 and \
                car_velocity.size > 900 and abs(turning_radians) < 0.1 or self.halfflipping:
