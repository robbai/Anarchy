...
def recover(self, car, rotation_velocity: Vector3, roll = True, pitch = True, yaw = True, allow_yaw_wrap: bool = True):
    wrap_yaw = (allow_yaw_wrap and abs(self.steer_correction_radians) > math.pi * 0.75 and pitch and yaw)
    if roll: self.controller.roll = clamp11(car.physics.rotation.roll * -3 + rotation_velocity.x * 0.3)
    if abs(car.physics.rotation.roll) < 1.5 or not roll:
        if pitch: self.controller.pitch = clamp11((car.physics.rotation.pitch - math.pi if wrap_yaw else car.physics.rotation.pitch) * -4 + rotation_velocity.y * 0.8)
...
def dodge(self, car, angle: float, rotation_velocity: Vector3, multiply = 1):
    if car.has_wheel_contact and not self.dodging:
        ...
        if self.time > self.next_dodge_time:
            self.controller.jump = True
            if car.has_wheel_contact or self.time > self.next_dodge_time + 1.5: self.dodging = False
        if self.time < self.next_dodge_time + 0.5:
            ...
            recover(self, rotation_velocity, yaw = (car.physics.location.z > 1000))


def halfflip(self, car, rotation_velocity: Vector3):
    if not self.halfflipping and car.has_wheel_contact:
