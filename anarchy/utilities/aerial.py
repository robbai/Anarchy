...
def default_pd(
    ...
    roll = steer_pd(
        -info.car.physics.rotation.roll, info.rotation_velocity.x / 6
    )  # keeps the bot upright
    if error == False:
...
class aerial_option_b:  # call at your own risk: yeets towards ball after taking a mostly wild guess at where it will be.
    ...
    def execute(
        self, packet: GameTickPacket, index, ball_prediction: BallPrediction = None
    ) -> SimpleControllerState:
        info = Info(packet, index)

        if (
            self.time == -9
        ):  # if we don't have a target time, guess one using really bad math
            ...
            if sign(before) == sign(after):  # sign returns 1 or -1
                eta = math.sqrt(
                    clamp(
                        ((info.ball_location - info.car_location).length + before)
                        / 529.165,
                        0.01,
                        9999,
                    )
                ...
                eta = math.sqrt(
                    clamp(
                        (
                            (info.ball_location - info.car_location).length
                            + before
                            + after
                        )
                        / 529.165,
            ...
            test = dpp3D(
                targetetaloc, info.ball_velocity, info.car_location, info.car_velocity
            )
            eta = math.sqrt(
                ((info.ball_location - info.car_location).length + max(0, test))
                / 529.165
            )
            ...
            if time_remain != 0:
                target = future(
                    info.ball_location, info.ball_velocity, time_remain, ball_prediction
                )
            else:
                time_remain = 0.1
                target = future(
                    info.ball_location, info.ball_velocity, 0.1, ball_prediction
                )

            # Shooting offset
            enemy_goal = Vector3(
                0,
                (1 if info.car.team == 0 else -1) * 5120,
                clamp(target.z / 2, 100, 600),
            )
            ...
            """
            target = backsolve_future(
                info.car_location, info.car_velocity, target, time_remain, radius=0
            )
