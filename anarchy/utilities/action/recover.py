...
class Recover(ActionBase):
    ...
    def step(self, packet: GameTickPacket) -> SimpleControllerState:
        rotation_velocity = self.agent.rotation_velocity
        ...
        if self.roll: controller.roll = clamp11(self.agent.car.physics.rotation.roll * -3 + rotation_velocity.x * 0.3)
        if abs(self.agent.car.physics.rotation.roll) < 1.5 or not self.roll:
            if self.pitch:
                controller.pitch = clamp11((self.agent.car.physics.rotation.pitch - math.pi if wrap_yaw else self.agent.car.physics.rotation.pitch) * -4 + rotation_velocity.y * 0.8)
            if self.yaw:
                yaw = invert_angle(self.agent.steer_correction_radians) if wrap_yaw else self.agent.steer_correction_radians
                controller.yaw = clamp11(yaw * 3.75 - rotation_velocity.z * 0.95)
