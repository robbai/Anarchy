...
class Recover(ActionBase):
    ...
    def step(self, packet: GameTickPacket) -> SimpleControllerState:
        ...
        if self.roll: controller.roll = clamp11(self.agent.car.physics.rotation.roll * -3 + self.agent.rotation_velocity.x * (10 ** 10 if wrap_yaw else 0.3))
        if abs(self.agent.car.physics.rotation.roll) < 1.5 or not self.roll:
            if self.pitch:
                controller.pitch = clamp11((self.agent.car.physics.rotation.pitch - math.pi if wrap_yaw else self.agent.car.physics.rotation.pitch) * -4 + self.agent.rotation_velocity.y * 0.8)
            if self.yaw:
